...
class ColorLib(object):
	...
	def _normalizeColorName(self, color=None):
		...
		if color == 'gray': color = 'light gray'
		if len(color) == 1 and self.codes.has_key(color): color = self.codes[color]
	...
	def rainbow(self, text=None, offset=None, style='rainbow'):
		...
		for line in text.splitlines():
			...
			for char in line:
				...
				if char.isspace() is True:
					...
					output += self.getColor(fg=color) + char + self.reset()
