...
# sprint PCS bullshit
spcs_piclink = re.compile(r'<a .*?href="(http://pictures.sprintpcs.com/share.'
                          r'do?.*?)">View Picture</a>', re.DOTALL)
spcs_thepic = re.compile(r'<img class="guestMediaImg".*?src="(http://pictures'
                         r'.sprintpcs.com(?::80)?/mmps/.*?)(?:\.jpg)?\?',
                         re.DOTALL + re.I)
...
class EmailGateway(object):
    ...
    _spams = ('This is an MMS message. Please go to http://mms.telusmobility.'
              'com/do/LegacyLogin to view the message.',
              'You have new Picture Mail!')
    _quoted = r'^(-+)\s*(original|forwarded)\s+(message|e?mail)\s*\1'
    _quoted = re.compile(_quoted, re.I)
    _sig = '--'
    _remove_quotes = re.compile('([\'"])(=\\?(?:[^\x00- ()<>@,;:"/\\[\\]?.=]+'
                                '\\?){2}[!->@-~]+\\?=)\\1')
