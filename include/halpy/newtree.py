...
class SQLTree(SQLBase):
    ...
    sql = dict(get_word='SELECT id FROM <TBL> WHERE word = %s AND parent = %s',
               ...
               get_tree='SELECT id FROM <TBL> WHERE parent = %s',
               get_symbol='SELECT word FROM <TBL> WHERE id = %s',
               )

    def get_symbol(self, node):
        with self.connection as cursor:
            cursor.execute(self.get_symbol_sql, (node,))
    ...
    def get_tree(self, node):
        with self.connection as cursor:
            cursor.execute(self.get_tree_sql, (node,))
...
class SQLDictionary(SQLBase):
    ...
    sql = dict(get_word='SELECT id FROM <TBL> WHERE word = %s',
               ...
               get_wordstr='SELECT word FROM <TBL> WHERE id = %s',
               )
    ...
    def __getitem__(self, id):
        with self.connection as cursor:
            cursor.execute(self.get_wordstr_sql, (id,))
