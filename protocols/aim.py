...
class AIMProtocol(Madcow):
    ...
    def run(self):
        ...
        #self.config.aim.username, self.config.aim.password
        self.proto = OSCARAuth('madcowbot3', 'rover42')
        self.proto.connect()
...
# XXX what is this?
            ...
            if requestClass == CAP_CHAT: # a chat request
                ...
                if not self.services.has_key(SERVICE_CHATNAV):
                    self.connectService(SERVICE_CHATNAV,1)#.addCallback(lambda x: self.services[SERVICE_CHATNAV].getChatInfo(exchange, name, instance).addCallback(self._cbGetChatInfoForInvite, user, moreTLVs[12]))
                else:
                    self.services[SERVICE_CHATNAV].getChatInfo(exchange, name, instance)#.addCallback(self._cbGetChatInfoForInvite, user, moreTLVs[12])
...
class ChatNavService(OSCARService):
    ...
    def getChatInfo(self, exchange, name, instance):
        #d = defer.Deferred()
        self.sendSNAC(0x0d,0x04,struct.pack('!HB',exchange,len(name)) + name + struct.pack('!HB',instance,2))##.addCallback(self._cbGetChatInfo, d)
        #return d
